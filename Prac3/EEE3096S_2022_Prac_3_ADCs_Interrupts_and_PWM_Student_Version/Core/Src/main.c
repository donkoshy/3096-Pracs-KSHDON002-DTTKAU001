...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  //TASK 2
	  adc_val = pollADC();		//gets the current adc value
	  sprintf(buffer, "A: %u \r\n", adc_val);
	  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 100);		//sends the adc val to termius


	  //TASK 3
	  crr_val = ADCtoCRR(adc_val);			//converts the adc val to crr
	  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, crr_val);
	  sprintf(buffer2, "P: %u \r\n", crr_val);
	  HAL_UART_Transmit(&huart2, buffer2, sizeof(buffer2), 100);	//sends the crr val to termius



	  //TASK 4
	  dutyCycle = (crr_val*100)/47999;			//calculates the duty cycle
	  sprintf(buffer3, "D: %u \r\n", dutyCycle);
	  HAL_UART_Transmit(&huart2, buffer3, sizeof(buffer3), 100);	//sends the duty cycle to termius
...
void EXTI0_1_IRQHandler(void)
{
	...
	if ((currentTime-previousTime > 200) && (B1_Pin == 1)){		//button is pressed
		if(delayTime == 1000){
			delayTime = 500;		//changes delaytime to 500 when button is pressed
		}
		else if (delayTime == 500){
			delayTime = 1000;		//changes delaytime to 1000 when button is pressed
		}
		previousTime = currentTime;		//sets the previoustime to currentime timestamp
	}
}
...
uint32_t pollADC(void){
	...
	HAL_ADC_Start(&hadc);		//starts the adc polling
	HAL_ADC_PollForConversion(&hadc, 100);
	uint32_t val = HAL_ADC_GetValue(&hadc);		//gets the current adc value to use for the ADCtoCRR funct.
	HAL_ADC_Stop(&hadc);		//stops polling the adc
	return val;
}
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	uint32_t val = (47999/4096)*adc_val;		//converts adc to crr using a simple ratio
	return val;
}
